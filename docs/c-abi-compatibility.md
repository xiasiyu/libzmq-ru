...
- `ZMQ_ROUTER_RAW` and `ZMQ_STREAM_NOTIFY` match original C ABI set-option validation; TCP `STREAM` sockets now expose routing-id/payload multipart receive frames and connect notifications for the active single-connection path. Full raw ROUTER/STREAM disconnect delivery remains outside the current guarantee.
...
- `ZMQ_HELLO_MSG` delivers configured lifecycle messages over inproc pipes and active TCP/IPC ZMTP handshake paths. `ZMQ_DISCONNECT_MSG` delivers receiver-side peer-disconnect messages over inproc pipes and active TCP/IPC ZMTP peer-close read paths. `ZMQ_HICCUP_MSG` delivers over active TCP/IPC ZMTP peer-close read paths.
...
- Fully asynchronous TCP/IPC lifecycle message delivery without an active socket operation remains incomplete.
