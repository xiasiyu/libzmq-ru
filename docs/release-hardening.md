...
- Hello lifecycle messages now deliver over inproc pipes and active TCP/IPC ZMTP handshake paths; receiver-side disconnect messages deliver over inproc pipes and active TCP/IPC ZMTP peer-close read paths. TCP/IPC hiccup delivery and fully asynchronous TCP/IPC lifecycle message delivery without an active socket operation remain incomplete.
