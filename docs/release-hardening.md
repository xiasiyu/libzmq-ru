...
- TCP `STREAM` active single-connection multipart receive/connect notification behavior is covered, but full STREAM disconnect notification delivery and ROUTER raw semantics remain incomplete.
...
- Hello lifecycle messages now deliver over inproc pipes and active TCP/IPC ZMTP handshake paths; receiver-side disconnect messages deliver over inproc pipes and active TCP/IPC ZMTP peer-close read paths; hiccup messages deliver over active TCP/IPC ZMTP peer-close read paths. Fully asynchronous TCP/IPC lifecycle message delivery without an active socket operation remains incomplete.
