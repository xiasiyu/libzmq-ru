...
- XPUB/XSUB draft option surface is partially covered, including `ZMQ_TOPICS_COUNT` and `ZMQ_ONLY_FIRST_SUBSCRIBE` validation. Inproc subscription notifications now suppress duplicate subscribes, aggregate topics across XSUB peers, use refcounted duplicate-subscribe unsubscribe behavior, and forward final unsubscribes. `ZMQ_XPUB_VERBOSE`/`ZMQ_XPUB_VERBOSER` forward duplicate subscribes, `ZMQ_XPUB_VERBOSER` forwards non-final multi-peer unsubscribes, and `ZMQ_XSUB_VERBOSE_UNSUBSCRIBE` forwards unmatched local unsubscribes. Full manual-last and only-first-subscribe multipart forwarding semantics still need parity work.
...
- Inproc XPUB/XSUB subscription forwarding now matches original first-subscribe, multi-peer duplicate suppression, refcounted final-unsubscribe notification behavior, XPUB verbose/verboser duplicate-subscribe behavior, XPUB verboser non-final unsubscribe behavior, and XSUB verbose unmatched-unsubscribe behavior for local inproc subscription changes.
