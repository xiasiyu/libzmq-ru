...
- Custom socket routing identity behavior still needs full original parity: C ABI option storage/validation, inproc plus active TCP/IPC `ZMQ_PROBE_ROUTER`, inproc `zmq_socket_get_peer_state` numeric plus exposed decimal-blob peer-id behavior, and TCP/IPC ZMTP READY `Identity` metadata exposure for UTF-8 routing ids are covered. Routing paths still use the current internal numeric peer-id model and do not yet match original TCP/IPC arbitrary binary blob identity framing.
- `ZMQ_ROUTER_RAW` and `ZMQ_STREAM_NOTIFY` now have original option-level validation coverage, and TCP `STREAM` covers routing-id/payload multipart receive frames plus connect notifications for the active single-connection path. Full raw ROUTER/STREAM disconnect delivery semantics are not complete yet.
- Draft hello options now deliver over the inproc pipe path, `ZMQ_DISCONNECT_MSG` now delivers receiver-side peer-disconnect messages over inproc and active TCP/IPC ZMTP peer-close read paths, `ZMQ_HICCUP_MSG` now delivers over active TCP/IPC ZMTP peer-close read paths, TCP/IPC `ZMQ_HELLO_MSG` is delivered on active ZMTP handshake paths, and hello/disconnect/hiccup options have original set/clear surface behavior. Fully asynchronous TCP/IPC lifecycle message delivery without an active socket operation is not complete yet.
...
- `ZMQ_HELLO_MSG` now delivers configured lifecycle messages over the inproc pipe path and active TCP/IPC ZMTP handshake paths. `ZMQ_DISCONNECT_MSG` now delivers receiver-side peer-disconnect messages over inproc and active TCP/IPC ZMTP peer-close read paths. `ZMQ_HICCUP_MSG` now delivers over active TCP/IPC ZMTP peer-close read paths.
...
- `ZMQ_PROBE_ROUTER` now sends the empty probe frame over inproc and active TCP/IPC ZMTP handshake paths.
- `ZMQ_ROUTER_RAW` and `ZMQ_STREAM_NOTIFY` now cover original C ABI set-option validation for ROUTER and STREAM sockets, and TCP `STREAM` covers active single-connection routing-id/payload multipart receive frames plus connect notifications.
