...
- Draft hello options now deliver over the inproc pipe path, inproc `ZMQ_DISCONNECT_MSG` now delivers receiver-side peer-disconnect messages, TCP/IPC `ZMQ_HELLO_MSG` is delivered on active ZMTP handshake paths, and hello/disconnect/hiccup options have original set/clear surface behavior. TCP/IPC disconnect/hiccup delivery and fully asynchronous TCP/IPC hello delivery without an active socket operation are not complete yet.
...
- `ZMQ_HELLO_MSG` now delivers configured lifecycle messages over the inproc pipe path and active TCP/IPC ZMTP handshake paths. `ZMQ_DISCONNECT_MSG` now delivers receiver-side peer-disconnect messages over the inproc pipe path.
