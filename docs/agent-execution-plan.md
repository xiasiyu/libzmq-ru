...
- [x] Implement GSSAPI client.
- [x] Implement GSSAPI server.
...
Status note: Phase 10 is in progress. NULL greeting behavior is used by the ZMTP transport. Z85 encode/decode, CURVE keypair generation, CURVE public-key derivation, ZAP request encoding, ZAP reply parsing, security socket option round-trips for PLAIN/CURVE/ZAP/GSSAPI-related options, PLAIN HELLO/WELCOME/INITIATE command handshakes, local PLAIN credential accept/reject behavior, external ZAP actor accept/deny flow for Rust-native and C ABI TCP PAIR, original-C++ PLAIN interop in both directions, NaCl-compatible CURVE HELLO/WELCOME/INITIATE/READY and encrypted MESSAGE body paths for Rust-native and C ABI TCP PAIR, original-C++ CURVE interop in both directions with CURVE-capable and secure oracle builds, CURVE local client-key reject behavior, CURVE ZAP actor accept behavior with raw 32-byte RFC 27 credentials, GSSAPI client/server sys bindings, platform `gss_init_sec_context`/`gss_accept_sec_context` token negotiation, GSSAPI READY/MESSAGE wrap/unwrap semantics when plaintext is disabled, baseline GSSAPI command/auth/ZAP paths for Rust-native and C ABI TCP PAIR, feature-disabled original-style GSSAPI placeholder INITIATE/READY framing for synthetic-principal tests, and zeroization of stored security material are implemented and tested. Original-C++ oracle builds are available at `../libzmq/build-ru-oracle-curve/lib/libzmq.dylib` and `../libzmq/build-ru-oracle-secure/lib/libzmq.dylib`; the secure oracle passes the existing NULL/PLAIN/CURVE interop checks. Real GSSAPI tests require `LIBZMQ_TEST_REAL_GSSAPI=1` plus a configured Kerberos principal/keytab or credential cache; without that environment, synthetic-principal tests skip under the real `gssapi` feature and continue to cover the compatibility placeholder in default-feature runs. Remaining work: original-C++ GSSAPI interop validation with configured Kerberos keytab/credential cache.
...
- [x] Implement inproc latency benchmark.
- [x] Implement inproc throughput benchmark.
- [x] Implement tcp latency benchmark.
...
- [x] Add unsafe counting command.
- [x] Add unsafe report.
- [x] Separate handwritten unsafe from generated bindings.
- [x] Verify default feature unsafe below 8%.
- [x] Verify full feature handwritten unsafe below 10%.
- [x] Audit every unsafe block for safety comments.
- [x] Move accidental unsafe from business logic into approved unsafe islands.

Completion checks:

- [x] Unsafe report generated.
- [x] Default feature unsafe target passes.
