...
impl Socket {
    ...
    pub fn connect(&self, endpoint: &str) -> Result<()> {
        ...
        self.send_inproc_hello_message()?;
    ...
    pub fn set_option_i32(&self, option: i32, value: i32) -> Result<()> {
        ...
        match option {
            ...
            ZMQ_ROUTER_RAW if self.socket_type == SocketType::Router && value >= 0 => {
                options.router_raw = value != 0;
            }
            ZMQ_ROUTER_MANDATORY => options.router_mandatory = value != 0,
            ZMQ_ROUTER_HANDOVER => options.router_handover = value != 0,
            ZMQ_STREAM_NOTIFY
                if self.socket_type == SocketType::Stream && matches!(value, 0 | 1) =>
            {
                options.stream_notify = value != 0;
            }
            ZMQ_REQ_CORRELATE => options.req_correlate = value != 0,
            ZMQ_REQ_RELAXED => options.req_relaxed = value != 0,
            ZMQ_XPUB_VERBOSE => {
                options.xpub_verbose = value != 0;
                let mut policy = self
                    .xpub_subscription_policy
                    .lock()
                    .map_err(|_| Error::InvalidSocket)?;
                policy.verbose_subscribe = value != 0;
                policy.verbose_unsubscribe = false;
            }
            ZMQ_XPUB_VERBOSER => {
                options.xpub_verboser = value != 0;
                let mut policy = self
                    .xpub_subscription_policy
                    .lock()
                    .map_err(|_| Error::InvalidSocket)?;
                policy.verbose_subscribe = value != 0;
                policy.verbose_unsubscribe = value != 0;
            }
            ...
            | ZMQ_IMMEDIATE
            | ZMQ_ROUTER_RAW
            | ZMQ_STREAM_NOTIFY => return Err(Error::InvalidArgument),
    ...
    fn send_inproc_hello_message(&self) -> Result<()> {
        let hello_msg = self
            .options
            .lock()
            .map_err(|_| Error::InvalidSocket)?
    ...
    fn send_inproc_lifecycle_message(&self, data: &[u8]) -> Result<()> {
        ...
        let mut message = Message::from_vec(data.to_vec());
        self.apply_outgoing_routing_id(&mut message)?;
        let outboxes = match self.resolve_outboxes(&message) {
    ...
    fn send_tcp_frame(&self, message: &Message) -> Result<()> {
        ...
        let options = self
            ...
            .map_err(|_| Error::InvalidSocket)?
            .clone();
        send_tcp_hello_message(&mut tcp, &options.hello_msg)?;
        send_tcp_probe_router(&mut tcp, self.socket_type, options.probe_router)?;
    ...
    fn send_ipc_frame(&self, message: &Message) -> Result<()> {
        ...
        let options = self
            ...
            .map_err(|_| Error::InvalidSocket)?
            .clone();
        send_ipc_hello_message(&mut ipc, &options.hello_msg)?;
        send_ipc_probe_router(&mut ipc, self.socket_type, options.probe_router)?;
    ...
    fn emit_monitor_pipe_stats(&self) -> Result<()> {
        ...
        if self
            ...
            .is_some()
        {
            let endpoint_uri = self.last_endpoint_string()?;
            Self::push_monitor_event_v2(
                &monitor_outbox,
                ZMQ_EVENT_PIPES_STATS as u64,
                &[0, 0],
    ...
    fn last_endpoint_string(&self) -> Result<String> {
        let endpoint = self
            .options
            .lock()
            .map_err(|_| Error::InvalidSocket)?
            .last_endpoint
            .clone();
        Ok(String::from_utf8_lossy(&endpoint).into_owned())
    }
...
fn send_tcp_hello_message(tcp: &mut TcpState, hello_msg: &[u8]) -> Result<()> {
    ...
    write_tcp_message_frame(tcp, hello_msg)?;
    tcp.hello_sent = true;
    Ok(())
}

fn send_tcp_probe_router(
    tcp: &mut TcpState,
    socket_type: SocketType,
    probe_router: bool,
) -> Result<()> {
    if tcp.probe_router_sent
        || !probe_router
        || !matches!(socket_type, SocketType::Dealer | SocketType::Router)
    {
        return Ok(());
    }
    write_tcp_message_frame(tcp, &[])?;
    tcp.probe_router_sent = true;
    Ok(())
}
...
fn write_tcp_message_frame(tcp: &mut TcpState, data: &[u8]) -> Result<()> {
    if tcp.curve_session.is_some() {
        let frame = {
            let session = tcp.curve_session.as_mut().ok_or(Error::InvalidSocket)?;
            curve_message_frame(session, data, false)?
        };
        let stream = tcp.stream.as_mut().ok_or(Error::Again)?;
        ...
        let frame = {
            let session = tcp.gssapi_session.as_mut().ok_or(Error::InvalidSocket)?;
            gssapi_message_frame(session, data, false)?
        };
        let stream = tcp.stream.as_mut().ok_or(Error::Again)?;
        stream.write_all(&frame).map_err(map_io_error)?;
    } else {
...
fn send_ipc_hello_message(ipc: &mut IpcState, hello_msg: &[u8]) -> Result<()> {
    ...
    write_ipc_message_frame(ipc, hello_msg)?;
    ipc.hello_sent = true;
    Ok(())
}

fn send_ipc_probe_router(
    ipc: &mut IpcState,
    socket_type: SocketType,
    probe_router: bool,
) -> Result<()> {
    if ipc.probe_router_sent
        || !probe_router
        || !matches!(socket_type, SocketType::Dealer | SocketType::Router)
    {
        return Ok(());
