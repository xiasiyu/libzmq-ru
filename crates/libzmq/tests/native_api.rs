...
#[test]
fn native_tcp_hiccup_msg_delivers_after_peer_disconnect() {
    let port = unused_tcp_port();
    let endpoint = format!("tcp://127.0.0.1:{port}");
    let ctx = Context::new().unwrap();
    let server = ctx.socket(SocketType::Server).unwrap();
    ...
    client.set_option_bytes(ZMQ_HELLO_MSG, b"hello").unwrap();
    client.set_option_bytes(ZMQ_HICCUP_MSG, b"hiccup").unwrap();
    server.bind(&endpoint).unwrap();
    client.connect(&endpoint).unwrap();
    ...
    assert_eq!(recv_retry_native(&server).unwrap().data(), b"hello");
    assert_eq!(recv_retry_native(&server).unwrap().data(), b"payload");
    server.unbind(&endpoint).unwrap();
    assert_eq!(recv_retry_native(&client).unwrap().data(), b"hiccup");
}
...
#[cfg(unix)]
#[test]
fn native_ipc_hiccup_msg_delivers_after_peer_disconnect() {
    let path = std::env::temp_dir().join(format!(
        "libzmq-native-ipc-{}-hiccup-msg.sock",
        std::process::id()
    ));
    let _ = std::fs::remove_file(&path);
    let endpoint = format!("ipc://{}", path.display());
    ...
    assert_eq!(server.recv().unwrap().data(), b"payload");
    server.unbind(&endpoint).unwrap();
    assert_eq!(client.recv().unwrap().data(), b"hiccup");
    let _ = std::fs::remove_file(path);
}
